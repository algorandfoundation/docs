...
parser = argparse.ArgumentParser(
    description="Reformat markdown files to display in mkdocs."
)
parser.add_argument("-doc-dir", required=True, help="Path to document directory.")
parser.add_argument(
    "-cmd",
    required=False,
    help="Full path to command. If provided the doc-dir will be emptied and new markdown files will be generated with 'cmd generate-docs <doc-dir>'",
)
...
def process_markdown_file(new_file, original_name, goal_depth, with_subcommand):
    ...
    with tempfile.NamedTemporaryFile(mode="w", dir=".", delete=False) as tmp, open(
        new_file, "r"
    ) as f:
...
def fix_root(num_files_modified, path):
    ...
    if num_files_modified == 1 and len(files) == 1:
        ...
        pp.pprint(p)
        shutil.move(
            os.path.join(path, files[0]), os.path.join(Path(path).parents[0], files[0])
        )
        shutil.rmtree(path)
        return
