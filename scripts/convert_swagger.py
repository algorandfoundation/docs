...
parser = argparse.ArgumentParser(description='Generate markdown files from swagger specfile.')
parser.add_argument('-processors', required=False, help='Optional file full of commands to fix broken swagger files. Replaces occurrances of \'{specfile}\' with the specfile')
parser.add_argument('-target', required=True, help='Full path to output file, including the name.')
...
def convert_swagger_to_markdown(specfile, directory):
    ...
    command = 'java -jar %s convert --swaggerInput %s --config %s --outputDir %s' % (convert_command, specfile, config, directory)
