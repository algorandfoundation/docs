...
The endpoint for the REST API is defined in the [Node Configuration Settings](https://developer.algorand.org/docs/run-a-node/reference/config/) by the property `EndpointAddress`. By default, the host is set to `127.0.0.1` and the port is either `8080` or, if `8080` is not available, chosen randomly.
