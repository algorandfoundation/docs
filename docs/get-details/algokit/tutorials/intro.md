...
This guide presents installing AlgoKit using an OS agnostic procedure. For OS specific instructions take a look that the AlgoKit install guides available for [macOS](./install-macos.md), [Windows](./install-windows.md) and [Linux](./install-linux.md).
...
AlgoKit supports using a [local version of the Algorand blockchain](../cli/localnet.md). To start an instance of this LocalNet run the following command from the terminal:
