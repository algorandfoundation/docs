...
Since keys are not stored on disk, standalone accounts can be used in [secure offline signing procedures](/get-details/transactions/offline_transactions) where hardware constraints may make using kmd more difficult.
...
!!! tip
	Since multisignature accounts are just logical representations of the data defined above, anyone can "create" the same Algorand address if they know how it is composed. This information is public and included in a signed transaction from a multisignature account. See [how multisignatures look in a signed transaction](/get-details/transactions/signatures#multisignatures).
