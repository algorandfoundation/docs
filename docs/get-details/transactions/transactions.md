...
|<a name="genesishash">GenesisHash</a>|_required_|[32]byte|`"gh"`|The hash of the genesis block of the network for which the transaction is valid. See the genesis hash for [MainNet](/get-details/algorand-networks/mainnet#genesis-hash), [TestNet](/get-details/algorand-networks/testnet#genesis-hash), and [BetaNet](/get-details/algorand-networks/betanet#genesis-hash).
...
|<a name="genesisid">GenesisID</a>|_optional_|string|`"gen"`| The human-readable string that identifies the network for the transaction. The genesis ID is found in the genesis block. See the genesis ID for [MainNet](/get-details/algorand-networks/mainnet#genesis-id), [TestNet](/get-details/algorand-networks/testnet#genesis_id), and [BetaNet](/get-details/algorand-networks/betanet#genesis-id). |
<a name="group">Group</a>|_optional_|[32]byte|`"grp"`|The group specifies that the transaction is part of a group and, if so, specifies the hash of the transaction group. Assign a group ID to a transaction through the workflow described in the [Atomic Transfers Guide](/get-details/atomic_transfers).|
<a name="lease">Lease</a>|_optional_|[32]byte|`"lx"`|A lease enforces mutual exclusion of transactions. If this field is nonzero, then once the transaction is confirmed, it acquires the lease identified by the (Sender, Lease) pair of the transaction until the LastValid round passes. While this transaction possesses the lease, no other transaction specifying this lease can be confirmed.  A lease is often used in the context of Algorand Smart Contracts to prevent replay attacks. Read more about [Algorand Smart Contracts](/get-details/dapps/pyteal/smart-contracts/) and see the Delegate [Key Registration TEAL template](/get-details/dapps/avm/teal/templates/delegate_keyreg) for an example implementation of leases. Leases can also be used to safeguard against unintended duplicate spends. For example, if I send a transaction to the network and later realize my fee was too low, I could send another transaction with a higher fee, but the same lease value. This would ensure that only one of those transactions ends up getting confirmed during the validity period. |
|<a name="note">Note</a>|_optional_|[]byte|`"note"`| Any data up to 1000 bytes. |
|<a name="rekeyto">RekeyTo</a>|_optional_|Address|`"rekey"`| Specifies the authorized address. This address will be used to authorize all future transactions. Learn more about [Rekeying](/get-details/accounts/rekey/) accounts. |
...
| <a name="">OnComplete</a>| _required_| uint64| `"apan"`| Defines what additional actions occur with the transaction. See the [OnComplete](/get-details/dapps/avm/teal/specification#oncomplete) section of the TEAL spec for details.|
