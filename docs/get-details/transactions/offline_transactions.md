...
The same methodology described here can also be used to work with [LogicSignatures](../dapps/pyteal/smart-contracts/stateless/modes#logic-signatures) and [Multisignatures](signatures#multisignatures). All objects in the following examples use msgpack to store the transaction object ensuring interoperability with the SDKs and `goal`.
