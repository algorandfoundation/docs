...
See the [*TEAL Reference Guide*](/get-details/dapps/avm/teal/specification) to understand how to write TEAL and the [*TEAL Opcodes*](/get-details/dapps/avm/teal/opcodes) documentation that describes the opcodes available. This guide assumes that the reader is familiar with [TEAL](/get-details/dapps/avm/teal/).
...
The [TEAL opcodes](/get-details/dapps/avm/teal/opcodes) documentation describes all transaction properties that are available within a TEAL program. These properties can be retrieved using TEAL.
...
In the above example, the second transaction’s type is checked, where the `int pay` references a payment transaction. See the [opcodes](/get-details/dapps/avm/teal/opcodes) documentation for all transaction types. Note that the `gtxn` call is a zero-based index into the atomic group of transactions. The `gtxns` opcode could also have been used to retrieve the index into the atomic group from the top of the stack instead of hard coding the index. If the TEAL program fails, all transactions in the group will fail.
...
This call returns two values. The first is a 0 or 1 indicating if the parameter was found and the second contains the value of the parameter. See the [opcodes](/get-details/dapps/avm/teal/opcodes) documentation for more details on what additional parameters can be read.
