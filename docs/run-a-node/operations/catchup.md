...
!!! info
    If interested in getting a build that provides quick access to begin development take a look at [Fast Catchup](/run-a-node/setup/install/#sync-node-network-using-fast-catchup),  [Docker Sandbox](/archive/build-apps/setup/#2-use-docker-sandbox) or use a [Third-party](/archive/build-apps/setup/#1-use-a-third-party-service) service. 
