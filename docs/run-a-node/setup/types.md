...
# Archival Mode
 ...
 The archival property must be set to true to run in archival mode, which will then set the node to store the entire ledger. Visit the [Node Configuration](/run-a-node/reference/config) guide for details on configuring your node. 
...
See [Configuring Node as a Relay](/run-a-node/reference/relay) for more information on setting up a relay.
