...
To turn on indexing for a node, the `isIndexerActive` configuration parameter must be set to `true`. The [Node Configuration](/run-a-node/reference/config) guide describes setting node configuration properties.
