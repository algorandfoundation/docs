...
|---|---|---|---|---|
|<a name="sender">Sender</a>| _required_ |basics.Address|`"snd"`|The address of the account that pays the fee and amount.|
|<a name="fee">Fee</a>| _required_| basics.MicroAlgos|`"fee"`|Paid by the sender to the FeeSink to prevent denial-of-service. The minimum fee on Algorand is currently 1000 microAlgos.|
|<a name="firstvalid">FirstValid</a>| _required_ | basics.Round | `"fv"`|The first round for when the transaction is valid. If the transaction is sent prior to this round it will be rejected by the network.|
|<a name="lastvalid">LastValid</a>| _required_ | basics.Round | `"lv"`|The ending round for which the transaction is valid. After this round, the transaction will be rejected by the network.|
...
|<a name="genesishash">GenesisHash</a>|_required_|crypto.Digest|`"gh"`|The base64-encoded SHA512/256 hash of the genesis block of the network for which the transaction is valid. See the genesis hash for [MainNet](../algorand-networks/mainnet.md#genesis-hash), [TestNet](../algorand-networks/testnet.md#genesis-hash), and [BetaNet](../algorand-networks/betanet.md#genesis-hash).
<a name="group">Group</a>|_optional_|crypto.Digest|`"grp"`|The group specifies that the transaction is part of a group and if so specifies the hash of the transaction group. Assign a group ID to a transaction through the workflow described in the [Atomic Transfers Guide](../feature-guides/atomic_transfers.md).|
...
|<a name="type">TxType</a>|_required_|protocol.TxType|`"type"`| Specifies the type of transaction. This value is automatically generated using any of the developer tools.
...
|Field|Required|Type|codec| Description|
|---|---|---|---|---|
|<a name="receiver">Receiver</a>| _required_ |basics.Address|`"rcv"`|The address of the account that receives the [amount](#amount).|
|<a name="amount">Amount</a>|_required_|basics.MicroAlgos|`"amt"`| The total amount to be sent in microAlgos.|
|<a name="closeremainderto">CloseRemainderTo</a>|_optional_|basics.Address|`"close"`|When set, it indicates that the transaction is requesting that the [Sender](#sender) account should be closed, and all remaining funds, after the [fee](#fee) and [amount](#amount) are paid, be transferred to this address.|
...
|Field|Required|Type|codec| Description|
|---|---|---|---|---|
|<a name="votepk">VotePk</a>| _required for online_ |crypto.OneTimeSignatureVerifier|`"votekey"`|A base64 encoded string corresponding to the root participation public key. See [Generate a Participation Key](../network-participation/participate-in-consensus/generate_keys.md) to learn more.|
|<a name="selectionpk">SelectionPK</a>|_required for online_|crypto.VRFVerifier|`"selkey"`| A base64-encoded string corresponding to the VRF public key.|
|<a name="votefirst">VoteFirst</a>|_required for online_|basics.Round|`"votefst"`|The first round that the *participation key* is valid. Not to be confused with the [FirstValid](#firstvalid) round of the keyreg transaction.|
|<a name="votelast">VoteLast</a>|_required for online_|basics.Round|`"votelst"`|The last round that the *participation key* is valid. Not to be confused with the [LastValid](#lastvalid) round of the keyreg transaction.|
...
|Field|Required|Type|codec| Description|
|---|---|---|---|---|
|<a name="configasset">ConfigAsset</a>| _required, except on create_ |basics.AssetIndex|`"caid"`|For re-configure or destroy transactions, this is the unique asset ID. On asset creation, the ID is set ot zero.|
|[AssetParams](#asset-parameters-assetparams) except on destroy_|basics.AssetParams|`"apar"`| See AssetParams table for all available fields.|
...
|Field|Required|Type|codec| Description|
|---|---|---|---|---|
|<a name="creator">Creator</a>| _required on creation_ |basics.Address|`"creator"`|The address for the account that creates the asset. This is the address where the parameters for this asset can be foundn, and also the address where unwanted asset units can be sent to be destroyed.|
...
|<a name="metadatahash">URL</a>|_optional_|[]byte]|`"metadatahash"`| This field is intended to represent a base64-encoded 32-byte hash of some metadata that is relevant to your asset and/or asset holders. The format of this metadata is up to the application. This field can _only_ be specified upon creation. An example might be the hash of some certificate that acknowledges the digitized asset as the official representation of a particular real-world asset.  |
|<a name="manageraddr">ManagerAddr</a>|_optional_|basics.Address|`"managerkey"`| The address of the account who can manage the configuration of the asset and destroy it. |
|<a name="reserveaddr">ReserveAddr</a>|_optional_|basics.Address|`"reserveaddr"`| The address of the account that holds the reserve (non-minted) units of the asset. This address has no specific authority in the protocol itself. It is used in the case where you want to signal to holders of your asset that the non-minted units of the asset reside in an account that is different from the default [creator](#creator) account. |
|<a name="freezeaddr">FreezeAddr</a>|_optional_|basics.Address|`"freezeaddr"`| The address of the account used to freeze holdings of this asset. If empty, freeziingn is not permitted. |
|<a name="clawbackaddr">ManagerAddr</a>|_optional_|basics.Address|`"clawbackaddr"`| The address of the account who can clawback holdings of  this asset. If empty, clawback is not permitted. |
